...
export default function FilterPanel({ facets, onChange, filterDefs }: FilterPanelProps) {
  ...
  return (
    <div className="space-y-6">
      {facets.map(facet => {
        const facetName = filterDefs.find(def => def.filterField === facet.name)?.filterName || facet.name;

        const facetNameToDisplay = facetName?.includes('.') ? facetName.split('.')[0] || '' : facetName;

        return (
          <div key={filterDefs.find(def => def.filterField === facet.name)?.filterName || facet.name}>
            <h3 className="mb-2 font-semibold capitalize">{facetNameToDisplay}</h3>
            <div className="flex flex-col space-y-2">
              {facet.buckets.map(bucket => (
                <label key={bucket.value} className="inline-flex items-center">
                  <input type="checkbox" className="mr-2" onChange={() => handleCheckbox(facet.name, bucket.value)} />
                  <span>
