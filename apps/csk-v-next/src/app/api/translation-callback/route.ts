...
export const POST = async (req: NextRequest) => {
  const payload = (await req.json()) as TranslationRequestPayload;
  const translationPayload = payload.translationPayload;
  if (!translationPayload) {
    // eslint-disable-next-line no-console
    console.log('no translation payload');
    return Response.json({ updated: false }, { status: 500 });
  }
  ...
  const { translationMerged } = await mergeTranslationToUniform({
    // @ts-expect-error - canvasClient misalignment
    canvasClient,
    // @ts-expect-error - contentClient misalignment
    contentClient,
    translationPayload,
  ...
  return Response.json({ updated: translationMerged });
};
