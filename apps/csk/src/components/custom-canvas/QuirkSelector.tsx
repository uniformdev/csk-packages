...
import { FC, useMemo, useState, ChangeEvent, MouseEvent } from 'react';
...
const QuirkSelector: FC<QuirkSelectorProps & QuirkSelectorParams> = ({
  quirkId,
  fullWidth,
  textColor,
  textSize,
  ...
  const [selectedQuirk, setSelectedQuirk] = useState<string>(quirkId ? quirks?.[quirkId] : '');
  ...
  const onSave = async (e: MouseEvent<HTMLDivElement>) => {
    if (!quirkId) return;
    setIsSaving(true);
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <div className="flex flex-col gap-6">
      <select className={baseStyles} onChange={onChange} value={quirkId ? quirks?.[quirkId] : ''}>
      ...
      <div
        ...
        onClick={onSave}
      >
        <UniformSlot slot={slots.quirkSelectorSaveButton} />
      </div>
    </div>
  );
};

export default withFlattenParameters(QuirkSelector);
