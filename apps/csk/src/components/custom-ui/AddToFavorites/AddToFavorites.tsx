...
export const AddToFavorites: FC<AddToFavoritesProps> = ({
  ...
  const isInFavorites = productSlug ? storedFavorites[productSlug] : false;
  const label = isInFavorites ? 'Remove from favorites' : 'Add to favorites';

  const toggleFavorites = (e: MouseEvent<HTMLButtonElement>) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {addIcon && <Image src={addIcon} fill alt="Add to favorites" />}
        </div>
        ...
        >
          {removeIcon && <Image src={removeIcon} fill alt="Remove from favorites" />}
        </div>
      </button>
    </Container>
  );
