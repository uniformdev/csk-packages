...
const getLocalizedRoutePath = (path: string) => {
  return path?.replaceAll(':locale', locales?.defaultLocale);
};
...
export const getRecommendProductsFromCanvas = async ({
  ...
  const result = await resolveRouteFromRoutePath(getLocalizedRoutePath(USER_RECOMMENDATIONS_COMPOSITION_PATH));
...
export const getCartFromCanvas = async (): Promise<CartFromCanvas> => {
  const result = await resolveRouteFromRoutePath(getLocalizedRoutePath(CART_COMPOSITION_PATH));
