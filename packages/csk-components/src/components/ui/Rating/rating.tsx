...
export const Rating: FC<RatingProps> = ({ rating, showReviewLabel, starsColor, activeStarsColor }) => {
  const inactive = resolveColor(starsColor, 'fill');
  const active = resolveColor(activeStarsColor, 'fill');
  return (
    <div className="flex items-center space-x-1">
      {Array.from({ length: 5 }, (_, starIndex) => {
        const selectedRating = rating && rating >= starIndex + 1;
        const star = selectedRating ? active : inactive;
        return (
          <svg
            key={starIndex}
            className={cn('w-5 h-5', star.className)}
            style={star.style}
            aria-hidden="true"
